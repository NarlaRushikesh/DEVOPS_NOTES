...
## Deploying a Sample Manifest File
**Machine :** Master
```sh
...
node 'puppetagent' {
  file { '/tmp/puppet_test.txt':
    ...
    content => "Puppet is working on agent!",
  }

  notify { "Agent node applied successfully": }
}
```
**Machine :** Agent
```sh
...
Expected Output:
```
Agent node applied successfully

File created: /tmp/puppet_test.txt
```
